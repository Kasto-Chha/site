...
// Bold is tried before italic so **x** is not read as an italic "*x*" pair.
// Newlines are excluded from every marker, so an unclosed one can never swallow
// the rest of the answer — it just stays literal until its partner streams in.
const INLINE = /\*\*([^*\n]+)\*\*|`([^`\n]+)`|\*([^*\n]+)\*/;

// One line of text -> [{ text, bold?, italic?, code? }]
