...
export function searchTokens(text, { max = 6, minLength = 2 } = {}) {
  return [
    ...new Set(
      String(text || "")
        .slice(0, 200)
...
//
// A .or() string is passed through verbatim into a URL query parameter, and
// "%sandar%" reads as the start of a percent-escape there — %20 is a space, %2F
// a slash. "sa" is not valid hex, so decoders disagree about what to do with
// it, and the filter reaches Postgres partly mangled.
//
// The symptom is not a clean failure. Searching "momo" worked because "%mo"
// happens to decode harmlessly; "sandar" returned nothing, or one row out of
// three, or different results for the same request depending on who asked. That
// is much harder to spot than an outright error.
//
// PostgREST accepts `*` as the wildcard in URL filters for exactly this reason,
// and translates it to `%` server-side. Same query, no collision.
//
// This only affects .or(). A plain .ilike("col", "%term%") is fine, because
// supabase-js encodes that value itself.
