...
export function articleSchema(siteUrl, story) {
  ...
  return {
    ...
    // Google's own guidance for Article rich results treats an image as
    // close to required — an article without one is markedly less likely to
    // surface with a preview at all. Omitted rather than a placeholder when
    // a story has none, same as every optional field here.
