...
// The /featured listing page, paginated. Kept separate from
// getFeaturedStories() above rather than adding offset/limit to it, because
// that function's two other callers — the sitemap and the homepage — both
// want the unbounded set and would break if it suddenly stopped at a page.
//
// "slot" is an editorial pick of exactly one story ("main" vs "side" — see
// lib/admin/contentTypes.js), not a manual sort order, so it says nothing
// about how the other stories should be sequenced once there are more than
// fit on one page. Newest-first is the ordinary blog default and is what
// this uses; the main-slotted story is fetched separately and shown only on
// page 1, the way the homepage and the old unpaginated listing both already
// treated it as the lead.
export async function getFeaturedStoriesPage({ offset = 0, limit = 9 } = {}) {
  ...
  return {
    lead,
    rows,
    // A full page suggests there is at least one more — same reasoning as
    // getThreadPage's hasMore, and cheaper than a count query.
...
// A slug can be deliberately renamed later in the admin panel (not just
// auto-derived from the title — see 0006's slug-preserving trigger for that
// case). When a direct lookup above finds nothing, this checks whether the
// requested slug is actually an OLD one that's since been renamed, so the
// page can redirect to the current URL instead of showing "not found".
export function getFeaturedStoryByOldSlug(oldSlug) {
  return safeQuery(
    (supabase) =>
      supabase
        .from("featured_story_slug_history")
        .select("featured_stories(*)")
        .eq("old_slug", oldSlug)
        .single(),
    null
  );
...
export async function getThreadPage({ offset = 0, limit = 30 } = {}) {
  ...
  const allRows = await safeQuery((supabase) =>
    supabase
      .from("reviews")
      .select("*")
      .in("topic_slug", slugs)
  ...
  // A thread is in exactly one section at a time.
  //
  //   asked, not yet answered  ->  Community is Asking
  //   answered enough          ->  Discussions
  //
  // They are stages, not two views of the same list. An earlier version had
  // questions appear in both at once, which made the sections look like
  // duplicates of each other and stripped the meaning out of "still waiting on
  // someone who has been there".
  //
  // Only question-started threads wait. A shared experience goes straight to
  // Discussions however thin — somebody HAS been there, they just wrote it, so
  // parking it under a heading about unanswered questions would be false.
  //
  // The threshold is isDiscussionIndexable, the same function getRecentQuestions
  // uses. Sharing it is what guarantees the two sections cannot disagree: a
  // thread cannot be missing from both, or listed in both.
  const byThread = new Map();
  for (const row of allRows) {
    if (!byThread.has(row.topic_slug)) byThread.set(row.topic_slug, []);
    byThread.get(row.topic_slug).push(row);
  }

  const stillAsking = new Set();
  for (const [slug, group] of byThread) {
    const hasQuestion = group.some((row) => row.kind === "question");
    if (!hasQuestion) continue;
    const experiences = group.filter((row) => row.kind === "experience");
    if (!isDiscussionIndexable(experiences)) stillAsking.add(slug);
  }

  const rows = allRows.filter((row) => !stillAsking.has(row.topic_slug));
...
//
// `before` is a keyset cursor: the last_message_at of the oldest row the client
// already has. Paging that way rather than with an offset means a conversation
// bumped to the top while someone is scrolling can't shift the window and make
// a row appear twice or be skipped entirely — which is exactly what happens to
// a chat list, since answering in any thread reorders it.
export async function getUserChatTopics(userId, limit = 6, { before } = {}) {
  if (!userId) return [];
  return safeQuery((supabase) => {
    ...
    if (before) query.lt("last_message_at", before);
    return query;
  });
}
...
const TOPIC_COLUMNS = "id, title, message_count, created_at, last_message_at";

// Search over a user's own conversations — titles AND what was actually said
// in them.
//
// Title-only search looked like it worked and mostly didn't: a title is just
// the opening question truncated to 80 characters, so anything raised in a
// follow-up, and anything the assistant answered, was unfindable. Someone who
// remembers asking about a laptop warranty three weeks ago searches for
// "warranty" and gets nothing, because the conversation is titled after
// whatever they happened to open with.
//
// Two queries rather than one join: PostgREST cannot express "topics whose
// messages match" in a single request, and the message scan is already bounded
// to one user by idx_chat_messages_user.
export async function searchUserChatTopics(userId, term, limit = 20) {
  const query = (term || "").trim();
  if (!userId || !query) return [];

  // Remove every character that would be read as a wildcard: % and _ from SQL
  // LIKE, * because PostgREST rewrites it to %, and \ so an escape sequence
  // can't be smuggled in.
  //
  // This used to replace them with a space, which did not defuse anything — a
  // search for "%" became the pattern "% %", and that matches every title
  // containing a space, so typing one character returned the entire history.
  // Removing them outright means a term made only of wildcards has nothing
  // left to search for, which is no results rather than all of them.
  const cleaned = query.slice(0, 80).replace(/[%_*\\]/g, "").trim();
  if (!cleaned) return [];
  ...
  const [byTitle, byMessage] = await Promise.all([
    ...
    safeQuery((supabase) =>
      supabase
        ...
        .eq("user_id", userId)
        .ilike("content", like)
        // A term can appear in many turns of the same conversation, and only
        // the distinct topics matter. Over-fetch so a chatty thread cannot use
        // up the whole budget on its own.
        .order("created_at", { ascending: false })
        .limit(limit * 20)
    )
  ]);

  const seen = new Set(byTitle.map((row) => row.id));
  const extraIds = [];
  for (const row of byMessage) {
    if (!row?.topic_id || seen.has(row.topic_id)) continue;
    seen.add(row.topic_id);
    extraIds.push(row.topic_id);
    if (extraIds.length >= limit) break;
  }

  if (!extraIds.length) return byTitle;

  // The message hits give ids, not rows. Fetch the conversations themselves,
  // still scoped to the owner so a topic_id that somehow escaped the user
  // filter above cannot widen the result.
  const byContent = await safeQuery((supabase) =>
    supabase
      .from("chat_topics")
      .select(TOPIC_COLUMNS)
      .eq("user_id", userId)
  ...
  // Title matches first — the conversation is named after what it is about, so
  // those are the more likely hits — then the rest by recency.
