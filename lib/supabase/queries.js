...
// Shared experiences only. Question rows live in the same table (kind =
// "question") but are not experiences — they are the prompt a thread starts
// from, and counting them would make an unanswered question look answered.
  ...
  try {
    ...
    const [reviewCount, questionCount, chatCount, voteCount] = await Promise.all([
      ...
      supabase
        ...
        .select("*", { count: "exact", head: true })
        .eq("kind", "question"),
      // Questions asked of the assistant = user turns, not whole conversations.
      supabase
        .from("chat_messages")
        .select("*", { count: "exact", head: true })
        .eq("role", "user"),
